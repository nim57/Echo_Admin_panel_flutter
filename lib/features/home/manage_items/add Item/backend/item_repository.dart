...
class ItemRepository extends GetxController {
  ...
  Future<String> generateItemId() async { // Removed underscore to make public
    try {
      ...
      await _db.collection('counters').doc('itemId').set({'count': currentCount});
      return newId;
  ...
  Future<void> saveItem(Item item) async {
    try {
      ...
      await _db
          .collection('items')
          .doc(itemId)
          .set(item.copyWith(id: itemId).toJson());
    } on FirebaseException catch (e) {
      throw EFirebaseException(e.code).message;
    } on FormatException catch (_) {
      throw EFormatException('Format exception occurred');
  ...
  /// Get real-time stream of items by category
